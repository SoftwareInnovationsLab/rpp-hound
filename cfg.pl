...
% Iterates through each node in a path and outputs branch predicates.
get_path_conditions([(Node,Ctx)|Tail], Stream) :-
    prob(location(Node,File,Function,Line),_),
    (
        member((A,Ctx), Tail), prob(branch(Node,A,C),Prob), Prob > 0
        ->
        prob(branch_cond(Node,C,Cond),_),
        atomics_to_string([Node, ", ", File, ":", Function, ":", Line, ": ", Cond, "\n"], Output),
        write(Stream, Output),
        get_path_conditions(Tail, Stream)
        ;
        prob(branch(Node,A,C),Prob), Prob > 0, prob(loop(A),_) ->
        prob(branch_cond(Node,C,Cond),_),
        atomics_to_string([Node, ", ", File, ":", Function, ":", Line, ": ", Cond, "  %% avoids infinite loop\n"], Output),
        write(Stream, Output),
        get_path_conditions(Tail, Stream)
        ;
        get_path_conditions(Tail, Stream)
